...
import br.ihoje.cache.InMemoryCrawlerCache
import br.ihoje.crawler.LinkHarvesterSpec.LinkHarvesterFixture
...
class LinkHarvesterSpec extends ScalaTestWithActorTestKit with FixtureAsyncWordSpecLike:
  ...
  "LinkHarvester" should {
    "fetch all urls for a webpage from cache" in { f =>
      ...
      // When
      for _ <- crawlerCache.set(testUrl1, testUrl1Children)
      yield
